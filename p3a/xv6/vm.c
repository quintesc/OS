...
int          
s_mencrypt(char *virtual_addr, int len)
{
  if(len == 0)
    return 0;
  ...
  for(int i = 0; i < len; i++){
    ...
    for(int offset = 0; offset < PGSIZE; offset++){ // encrypt whole page
      page_slider[offset] = ~(page_slider[offset]);
...
mdecrypt(char *virtual_addr){
  ...
  // check for errors? TODO idk if we need this
  /*
...
int
s_getpgtable(struct pt_entry* entries, int num){
  if(num == 0)
    return 0;
...
int
s_dump_rawphymem(uint physical_addr, char *buffer) {
  if(physical_addr >= PHYSTOP)
    return -1;
  char *va = (char*)P2V(PGROUNDDOWN(physical_addr));
  int retval = copyout(myproc()->pgdir, (uint) buffer, (void *) va, (uint) PGSIZE);
  if (retval)
    return -1;
  return 0;
}
